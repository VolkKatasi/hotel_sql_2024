...
{
    public partial class infoStandart : Form
    {
        private readonly int m_nID;
        public infoStandart(int nID)
        {
            InitializeComponent();
            m_nID = nID;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (var connection = new SqliteConnection("Data Source=db/hotel.db"))
            {
                ...
                SqliteCommand cmd = new SqliteCommand($"select busy from stad where n like \"{numericUpDown1.Value}\";", connection);
                ...
                cmd.CommandText = $"UPDATE stad SET f_id=\"{m_nID}\", count=\"{numericUpDown2.Value}\", busy=\"1\" WHERE n=\"{numericUpDown1.Value}\"";
                cmd.ExecuteNonQuery();
