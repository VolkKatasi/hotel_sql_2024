...
{
    public partial class infoKomfo : Form
    {
        private readonly int m_nID;
        public infoKomfo(int nID)
        {
            InitializeComponent();
            m_nID = nID;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (var connection = new SqliteConnection("Data Source=db/hotel.db"))
            {
                ...
                SqliteCommand cmd = new SqliteCommand($"select busy from komf where n like \"{numericUpDown1.Value}\";", connection);
                ...
                cmd.CommandText = $"UPDATE komf SET f_id=\"{m_nID}\", count=\"{numericUpDown2.Value}\", busy=\"1\" WHERE n=\"{numericUpDown1.Value}\"";
                cmd.ExecuteNonQuery();
